    ...
    <div class="card flex-fill w-100">
        ...
        <div class="card-body py-3">
            <input type="text" placeholder="Pesquise aqui" class="form-control mb-3"
                aria-describedby="passwordHelpBlock" v-model="searchUsuario" />
            <div class="mt-4" v-for="item in usuariosOnCurrentPage" :key="item.id_user">

                <a class="row" style="text-decoration: none;">
                    <div class="col-3">
                        <div class="avatar-null rounded me-1" alt="Avatar" style="width: 50px; height: 50px;"> {{ item.iniciais }}</div>
                    </div>

                    <div class="col-9" style="margin-left: -5px;">
                        <h5><a href="#" style="text-decoration: none; color: #000;"><strong>{{
                            item.nome }} {{ item.sobrenome }}</strong></a>
                        </h5>
                        <h6><a href="#" style="text-decoration: none; color: #000; font-size: 10px; margin-top: -200px !important; font-weight: 100 !important;">{{
                            item.email }}</a>
                        </h6>
                        <h5><a href="#" style="text-decoration: none; color: #000;"><strong><span
                                        v-if="item.id_nivel === 1"
                                        class="badge text-bg-dark">Administrador</span></strong></a>
                        </h5>
                        <h5><a href="#" style="text-decoration: none; color: #000;"><strong><span
                                        v-if="item.id_nivel === 2" class="badge text-bg-info">Suporte /
                                        Atendimento</span></strong></a>
                        </h5>
                        <h5><a href="#" style="text-decoration: none; color: #000;"><strong><span
                                        v-if="item.id_nivel === 4"
                                        class="badge text-bg-info">Desenvolvedor</span></strong></a>
                        </h5>
            ...
            <div class="d-grid mt-3 mb-3 gap-2 d-md-flex justify-content-md-end">
                <button class="btn btn-dark btn-sm" @click="previousPageUser()" :disabled="currentPageUser <= 1">
                    Anterior
                </button>
                <button class=" btn btn-dark btn-sm" style="margin-right: 3% !important;" @click="nextPageUser()"
